...
export default function DemoCustomersPage() {
  const { customers: demoCustomers, updateCustomerDebt } = useDemo();

  const [activeTab, setActiveTab] = useState<Tab>("pelanggan");
  const [customerList, setCustomerList] = useState<Customer[]>(() => demoCustomers as Customer[]);
  const [payments, setPayments] = useState<Payment[]>([
    { id: "pay-1", customer_id: "demo-c1", amount: 50000, method: "cash", note: "Cicilan", created_at: "2024-03-15T10:00:00Z" },
    { id: "pay-2", customer_id: "demo-c4", amount: 100000, method: "transfer", note: "Cicilan bulan Maret", created_at: "2024-03-18T14:00:00Z" },
    { id: "pay-3", customer_id: "demo-c3", amount: 75000, method: "cash", note: "Bayar sebagian", created_at: "2024-03-20T09:00:00Z" },
  ]);
  ...
  const handleSaveCustomer = (e: React.FormEvent) => {
    ...
    if (editingCustomer) {
      setCustomerList(prev => prev.map(c => c.id === editingCustomer.id ? { ...c, name: formData.name, phone: formData.phone, address: formData.address } : c));
  ...
  const handleDelete = () => {
    ...
    setPayments(prev => prev.filter(p => p.customer_id !== deleteTarget.id));
  ...
  const handleAddDebt = () => {
    ...
    setCustomerList(prev => prev.map(c => c.id === showAddDebt.id ? { ...c, debt: newDebt } : c));
    updateCustomerDebt(showAddDebt.id, newDebt);
    const newPayment: Payment = {
      id: `pay-${Date.now()}`,
      customer_id: showAddDebt.id,
      amount: -amount,
      method: "hutang",
      note: addDebtNote || "Piutang baru",
      created_at: new Date().toISOString(),
    };
  ...
  const handlePayDebt = () => {
    ...
    setCustomerList(prev => prev.map(c => c.id === showPayDebt.id ? { ...c, debt: newDebt } : c));
    updateCustomerDebt(showPayDebt.id, newDebt);
    const newPayment: Payment = {
      id: `pay-${Date.now()}`,
            ...
            <div className="relative bg-white w-full sm:max-w-sm rounded-t-3xl sm:rounded-2xl shadow-2xl p-5 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {([["cash", "💵 Tunai"], ["transfer", "🏦 Transfer"]] as const).map(([m, label]) => (
    ...
    <div className="space-y-5 pb-20 md:pb-0">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 border border-[#E5E3DC] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#FF5F03]/20 focus:border-[#FF5F03] bg-white"
        />
      </div>
