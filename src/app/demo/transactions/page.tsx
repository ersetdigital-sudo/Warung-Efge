...
function pmColor(m: string) {
  if (m === "cash") return "bg-[#ECFDF5] text-[#16A34A] border-[#bbf7d0]";
  if (m === "qris") return "bg-[#EFF6FF] text-[#1D4ED8] border-[#bfdbfe]";
  if (m === "transfer") return "bg-[#F5F3FF] text-[#7C3AED] border-[#ddd6fe]";
  if (m === "edc") return "bg-[#FFFBEB] text-[#D97706] border-[#fde68a]";
  if (m === "hutang") return "bg-[#FEF2F2] text-[#DC2626] border-[#fecaca]";
...
function pmIcon(m: string) {
  ...
  if (m === "transfer") return CreditCard;
  return Smartphone;
}

export default function DemoTransactionsPage() {
  const { transactions: demoTransactions } = useDemo();
  ...
  const filtered = useMemo(() => {
    ...
    else if (sortBy === "highest") list.sort((a, b) => b.total - a.total);
    else if (sortBy === "lowest") list.sort((a, b) => a.total - b.total);
    return list;
  ...
  // Delete handler (local state only)
  ...
  const handleCetak = (trx: any) => {
    const items = (trx.transaction_items || []).map((i: any) => ({
      name: i.product_name,
      quantity: i.quantity,
      unit: i.unit || "pcs",
      price: i.price,
  ...
  return (
    <div className="space-y-4">
      {/* Toast */}
      {toast && (
        <div className="fixed z-[9999] top-4 right-4 px-4 py-2.5 rounded-xl shadow-xl text-sm font-medium text-white bg-[#DC2626] animate-in slide-in-from-top">
          {toast}
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
        <div className="bg-white border border-[#D9D6C8] rounded-md p-3 border-l-[3px] border-l-[#072C2C]">
          <p className="text-[10px] font-medium text-[#9CA3AF] uppercase tracking-wider">Total Transaksi</p>
          <p className="font-[Oswald] text-[22px] font-semibold text-[#072C2C] mt-1">{totalTrx}</p>
        </div>
        <div className="bg-white border border-[#D9D6C8] rounded-md p-3 border-l-[3px] border-l-[#16A34A]">
          <p className="text-[10px] font-medium text-[#9CA3AF] uppercase tracking-wider">Total Omzet</p>
          <p className="font-[Oswald] text-[22px] font-semibold text-[#072C2C] mt-1">{formatCurrency(totalOmzet)}</p>
        </div>
        <div className="bg-white border border-[#D9D6C8] rounded-md p-3 border-l-[3px] border-l-[#FF5F03]">
          <p className="text-[10px] font-medium text-[#9CA3AF] uppercase tracking-wider">Rata-rata / Trx</p>
          <p className="font-[Oswald] text-[22px] font-semibold text-[#072C2C] mt-1">{formatCurrency(avgTrx)}</p>
        </div>
        <div className="bg-white border border-[#D9D6C8] rounded-md p-3 border-l-[3px] border-l-[#D97706]">
          <p className="text-[10px] font-medium text-[#9CA3AF] uppercase tracking-wider">Metode Terpopuler</p>
          <p className="font-[Oswald] text-[22px] font-semibold text-[#072C2C] mt-1">{pmLabel(topPM)}</p>
        </div>
      </div>

      {/* Table Card */}
      <div className="bg-white border border-[#D9D6C8] rounded-xl shadow-sm overflow-hidden">
        {/* Toolbar */}
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-[12px]">
            ...
            <tbody>
              {paged.length === 0 && (
                <tr>
                  <td colSpan={9} className="text-center py-12 text-[#9CA3AF]">
                ...
                return (
                  <tr key={t.id} className="border-b border-[#D9D6C8] hover:bg-[#FAFAF8]">
                    <td className="px-3 py-2.5">
                      <span className="font-mono font-bold text-[11px] text-[#072C2C]">{t.transaction_number}</span>
                    </td>
                    <td className="px-3 py-2.5">
                      <div className="text-[11px]">{t.created_at ? formatDate(t.created_at) : "–"}</div>
                    ...
                    <td className="px-3 py-2.5">
                      <span className={`inline-flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded border ${pmColor(t.payment_method)}`}>
                        <Icon className="w-3 h-3" />
                        {pmLabel(t.payment_method)}
                      </span>
                    </td>
                    <td className="px-3 py-2.5">
                      <div className="text-[11px] text-[#072C2C] font-medium truncate max-w-[140px]">{itemLabel}</div>
                    </td>
                    <td className="px-3 py-2.5 text-right font-mono font-bold text-[11px] text-[#072C2C]">
                      {formatCurrency(t.total)}
                    </td>
                    <td className="px-3 py-2.5 text-right font-mono text-[11px] hidden sm:table-cell">
                      {formatCurrency(t.amount_paid || 0)}
                    </td>
                    <td className="px-3 py-2.5 text-right font-mono text-[11px] text-[#16A34A] hidden sm:table-cell">
                      {formatCurrency(t.change_amount || 0)}
                    </td>
                    <td className="px-3 py-2.5">
                      <div className="flex gap-1">
                        <button
                          onClick={() => setSelectedTrx(t)}
                          className="w-6 h-6 rounded border border-[#D9D6C8] flex items-center justify-center cursor-pointer hover:bg-[#ECFDF5] hover:border-[#bbf7d0] hover:text-[#16A34A] text-[#9CA3AF]"
                        >
                          <Eye className="w-3 h-3" />
                        </button>
                        <button
                          onClick={() => handleCetak(t)}
                          className="w-6 h-6 rounded border border-[#D9D6C8] flex items-center justify-center cursor-pointer hover:bg-[#FFF2EB] hover:border-[#fed7aa] hover:text-[#FF5F03] text-[#9CA3AF]"
                        >
                          <Download className="w-3 h-3" />
                        </button>
                        <button
                          onClick={() => handleDelete(t)}
                          className="w-6 h-6 rounded border border-[#D9D6C8] flex items-center justify-center cursor-pointer hover:bg-[#FEF2F2] hover:border-[#fecaca] hover:text-[#DC2626] text-[#9CA3AF]"
                        >
        ...
        <div className="md:hidden px-3 py-2 space-y-2">
          {paged.length === 0 && (
            <div className="text-center py-8 text-[#9CA3AF] text-sm">Belum ada transaksi</div>
          )}
          {paged.map((t) => {
            const items = t.transaction_items || [];
            const firstItem = items[0]?.product_name || "–";
              ...
              <div key={t.id} className="bg-white border border-[#D9D6C8] rounded-lg p-3 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-[#9CA3AF]">
                    {t.created_at ? `${formatDate(t.created_at)} ${new Date(t.created_at).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}` : ""}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    <button
                      onClick={() => setSelectedTrx(t)}
                      className="w-7 h-7 rounded border border-[#D9D6C8] flex items-center justify-center cursor-pointer text-[#9CA3AF]"
                    >
        ...
        <div className="flex items-center justify-between px-3 py-2 border-t border-[#D9D6C8] text-[11px] text-[#9CA3AF]">
          ...
          <div className="flex gap-1">
            <button
              ...
              className="px-2 py-1 rounded border border-[#D9D6C8] cursor-pointer hover:bg-[#EDEADE]"
            >
              ‹
            </button>
            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map(p => (
              <button
                ...
                className={`px-2 py-1 rounded border cursor-pointer ${
                  p === page
                    ? "bg-[#072C2C] text-white border-[#072C2C]"
                    : "border-[#D9D6C8] hover:bg-[#EDEADE]"
                }`}
              >
                {p}
              </button>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="sticky top-0 bg-white flex items-center justify-between px-5 py-3 border-b border-[#D9D6C8] rounded-t-2xl z-10">
              ...
              <button
                ...
                className="p-1.5 rounded-lg hover:bg-[#EDEADE] cursor-pointer"
              >
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#F5F5F0] border border-[#D9D6C8] rounded-lg p-3">
                  ...
                  <p className="text-xs font-medium text-[#072C2C] mt-1">
                    {selectedTrx.created_at
                      ? new Date(selectedTrx.created_at).toLocaleDateString("id-ID", { weekday: "long", day: "numeric", month: "long" })
                      : "–"}
                  </p>
                  <p className="text-[10px] text-[#9CA3AF]">
                    {selectedTrx.created_at ? new Date(selectedTrx.created_at).toLocaleTimeString("id-ID") : ""}
                  </p>
                </div>
                <div className="bg-[#F5F5F0] border border-[#D9D6C8] rounded-lg p-3">
              ...
              <div className="bg-[#F5F5F0] border border-[#D9D6C8] rounded-lg p-3 space-y-1.5">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-mono font-bold">{formatCurrency(selectedTrx.subtotal || selectedTrx.total)}</span>
                </div>
                {selectedTrx.discount > 0 && (
                  <div className="flex justify-between text-xs">
                    <span className="text-[#9CA3AF]">Diskon</span>
                    <span className="font-mono font-bold text-[#DC2626]">-{formatCurrency(selectedTrx.discount)}</span>
                  </div>
                )}
                <div className="flex justify-between text-sm font-bold pt-2 border-t border-[#D9D6C8]">
                  <span className="text-[#072C2C]">TOTAL</span>
                  <span className="font-mono text-[#072C2C]">{formatCurrency(selectedTrx.total)}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-[#9CA3AF]">Bayar</span>
                  <span className="font-mono font-bold">{formatCurrency(selectedTrx.amount_paid || 0)}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-[#9CA3AF]">Kembalian</span>
                  <span className="font-mono font-bold text-[#16A34A]">{formatCurrency(selectedTrx.change_amount || 0)}</span>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => handleCetak(selectedTrx)}
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-[#FF5F03] text-white font-bold text-xs rounded-lg cursor-pointer hover:bg-[#e55503]"
                >
                  <Download className="w-3.5 h-3.5" />
                  Cetak Struk
                </button>
                <button
                  onClick={() => handleDelete(selectedTrx)}
                  className="flex items-center justify-center gap-1.5 px-4 py-2.5 bg-[#DC2626] text-white font-bold text-xs rounded-lg cursor-pointer hover:bg-[#b91c1c]"
                >
                  <Trash2 className="w-3.5 h-3.5" />
                  Hapus
                </button>
                <button
                  onClick={() => setSelectedTrx(null)}
                  className="flex-1 py-2.5 border border-[#D9D6C8] text-[#072C2C]/60 font-medium text-xs rounded-lg cursor-pointer hover:bg-[#EDEADE]"
                >
                  Tutup
                </button>
              </div>
