...
function Badge({ children, variant }: { children: React.ReactNode; variant: "success" | "warning" | "danger" | "info" }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold border ${styles[variant]}`}>
      {children}
    </span>
  );
}
...
export default function DemoReportsPage() {
  const { products, transactions, customers, suppliers } = useDemo();
  ...
  const [expenses, setExpenses] = useState([
    { id: "exp-1", name: "Listrik & Air", amount: 450000 },
    { id: "exp-2", name: "Gaji Karyawan", amount: 3000000 },
    { id: "exp-3", name: "Sewa Tempat", amount: 1500000 },
  ]);
  ...
  const pmData = useMemo(() => {
    const map: Record<string, number> = {};
    filteredTransactions.forEach((t: any) => { const m = t.payment_method || "cash"; map[m] = (map[m] || 0) + 1; });
    ...
    return [
      ...
      { name: "Hutang", value: Math.round(((map.hutang || 0) / total) * 100), color: "#DC2626" },
    ].filter(p => p.value > 0);
  }, [filteredTransactions]);
  ...
  // === STOK DATA ===
  const stokAman = products.filter((p: any) => p.stock > p.min_stock).length;
  const stokMenipis = products.filter((p: any) => p.stock > 0 && p.stock <= p.min_stock).length;
  const stokHabis = products.filter((p: any) => p.stock <= 0).length;
  const expiredSoon = products.filter((p: any) => p.expiry_date && Math.ceil((new Date(p.expiry_date).getTime() - Date.now()) / 86400000) <= 30).length;
  ...
  const handleAddExp = () => {
    ...
    setExpenses(prev => [...prev, { id: `exp-${Date.now()}`, name: newExpName, amount: Number(newExpAmount) }]);
    setNewExpName("");
    setNewExpAmount("");
    setShowAddExp(false);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === "penjualan" && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#FF5F03]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Total Penjualan</p><Wallet className="w-4 h-4 text-[#FF5F03]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{formatCurrency(totalSales)}</p></div>
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#072C2C]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Total Transaksi</p><Receipt className="w-4 h-4 text-[#072C2C]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{totalTrx}</p></div>
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#16A34A]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Rata-rata / Trx</p><Calculator className="w-4 h-4 text-[#16A34A]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{formatCurrency(avgTrx)}</p></div>
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#D97706]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Metode Terpopuler</p><ShoppingCart className="w-4 h-4 text-[#D97706]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{pmData[0]?.name || "–"}</p></div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-3">
            ...
            <div className="bg-white border border-[#D9D6C8] rounded-lg">
              ...
              <div className="p-4 h-[160px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={(() => {
                    ...
                    filteredTransactions.forEach((t: any) => { const h = new Date(t.created_at).getHours(); hourMap[h] = (hourMap[h] || 0) + 1; });
                    ...
                    <Bar dataKey="trx" radius={[3, 3, 0, 0]}>
                      {Array.from({ length: 16 }, (_, i) => i + 6).map((h, i) => {
                        const hourMap: Record<number, number> = {};
                        filteredTransactions.forEach((t: any) => { const hr = new Date(t.created_at).getHours(); hourMap[hr] = (hourMap[hr] || 0) + 1; });
              ...
              <div className="px-4 pb-3 space-y-1.5">
                {(() => {
                  const pagi = filteredTransactions.filter((t: any) => { const h = new Date(t.created_at).getHours(); return h >= 6 && h < 12; }).length;
                  const siang = filteredTransactions.filter((t: any) => { const h = new Date(t.created_at).getHours(); return h >= 12 && h < 17; }).length;
                  const sore = filteredTransactions.filter((t: any) => { const h = new Date(t.created_at).getHours(); return h >= 17 && h < 22; }).length;
      ...
      {tab === "produk" && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#FF5F03]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Total SKU</p><Package className="w-4 h-4 text-[#FF5F03]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{products.length}</p></div>
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#072C2C]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Total Unit Terjual</p><TrendingUp className="w-4 h-4 text-[#072C2C]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{topProducts.reduce((s, p) => s + p.sold, 0).toLocaleString("id-ID")}</p></div>
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#16A34A]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Produk Terlaris</p><BarChart3 className="w-4 h-4 text-[#16A34A]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C] truncate">{topProducts[0]?.name || "–"}</p><p className="text-[9px] text-[#9CA3AF]">{topProducts[0]?.sold || 0} unit terjual</p></div>
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#DC2626]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Stok Habis</p><AlertTriangle className="w-4 h-4 text-[#DC2626]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#DC2626]">{stokHabis} produk</p></div>
          </div>
          ...
          {(() => {
            ...
            const expiring = products.filter((p: any) => p.expiry_date).map((p: any) => ({ ...p, daysLeft: Math.ceil((new Date(p.expiry_date).getTime() - today.getTime()) / (1000 * 60 * 60 * 24)) })).filter((p: any) => p.daysLeft <= 30).sort((a: any, b: any) => a.daysLeft - b.daysLeft);
      ...
      {tab === "kasir" && (
        <div className="space-y-4">
          {/* KPI */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#FF5F03]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Total Kasir Aktif</p><Users className="w-4 h-4 text-[#FF5F03]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{kasirData.length} kasir</p></div>
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#072C2C]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Transaksi Terbanyak</p><Receipt className="w-4 h-4 text-[#072C2C]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{kasirData[0]?.name || "–"}</p><p className="text-[9px] text-[#9CA3AF]">{kasirData[0]?.trx || 0} transaksi</p></div>
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#16A34A]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Pendapatan Tertinggi</p><Wallet className="w-4 h-4 text-[#16A34A]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{formatCurrency(kasirData[0]?.total || 0)}</p><p className="text-[9px] text-[#9CA3AF]">{kasirData[0]?.name || "–"}</p></div>
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#D97706]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Total Omzet</p><DollarSign className="w-4 h-4 text-[#D97706]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{formatCurrency(kasirData.reduce((s, k) => s + k.total, 0))}</p></div>
          </div>
          ...
          <div className="bg-white border border-[#D9D6C8] rounded-lg">
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-[12px]">
                ...
                <tbody>
                  ...
                  {filteredTransactions.slice(0, 12).map((t: any) => {
                    const pm = t.payment_method || "cash";
                    const pmConfig: Record<string, { label: string; bg: string; text: string }> = {
                      cash: { label: "Tunai", bg: "bg-green-50", text: "text-green-700" },
                      qris: { label: "QRIS", bg: "bg-blue-50", text: "text-blue-700" },
                      transfer: { label: "Transfer", bg: "bg-purple-50", text: "text-purple-700" },
                      hutang: { label: "Hutang", bg: "bg-red-50", text: "text-red-700" },
                      edc: { label: "EDC", bg: "bg-amber-50", text: "text-amber-700" },
                    };
                    ...
                    return (
                      <tr key={t.id} className="border-b border-[#D9D6C8] hover:bg-[#FAFAF8]">
                        ...
                        <td className="px-3 py-2"><span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-green-50 text-green-600">Selesai</span></td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="md:hidden divide-y divide-[#F0EEE8]">
              ...
              {filteredTransactions.slice(0, 12).map((t: any) => {
                const pm = t.payment_method || "cash";
                const pmCfg: Record<string, { label: string; bg: string; text: string }> = {
                  cash: { label: "Tunai", bg: "bg-green-50", text: "text-green-700" },
                  qris: { label: "QRIS", bg: "bg-blue-50", text: "text-blue-700" },
                  transfer: { label: "Transfer", bg: "bg-purple-50", text: "text-purple-700" },
                  hutang: { label: "Hutang", bg: "bg-red-50", text: "text-red-700" },
      ...
      {tab === "stok" && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#072C2C]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Total SKU</p><Boxes className="w-4 h-4 text-[#072C2C]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{products.length}</p></div>
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#16A34A]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Stok Aman</p><Package className="w-4 h-4 text-[#16A34A]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#16A34A]">{stokAman}</p></div>
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#D97706]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Stok Menipis / Habis</p><AlertTriangle className="w-4 h-4 text-[#D97706]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#D97706]">{stokMenipis + stokHabis}</p><p className="text-[9px] text-[#9CA3AF]">{stokMenipis} menipis · {stokHabis} habis</p></div>
            <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#8B5CF6]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Mau Kadaluarsa</p><AlertTriangle className="w-4 h-4 text-[#8B5CF6]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#8B5CF6]">{expiredSoon}</p><p className="text-[9px] text-[#9CA3AF]">dalam 30 hari</p></div>
          </div>
          ...
          <div className="bg-white border border-[#D9D6C8] rounded-lg">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-[12px]">
                ...
                <tbody>
                  {[...products].sort((a: any, b: any) => (a.stock / (a.min_stock || 1)) - (b.stock / (b.min_stock || 1))).map((p: any) => {
          ...
          {(() => {
            ...
            const expiring = products
              ...
              .map((p: any) => ({ ...p, daysLeft: Math.ceil((new Date(p.expiry_date).getTime() - now) / 86400000) }))
      ...
      {tab === "keuangan" && (() => {
        ...
        const totalHPP = filteredTransactions.reduce((s: number, t: any) => {
        ...
        return (
          <div className="space-y-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
              <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#16A34A]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Total Pendapatan</p><TrendingUp className="w-4 h-4 text-[#16A34A]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{formatCurrency(totalSales)}</p></div>
              ...
              <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#FF5F03]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Laba Bersih</p><Wallet className="w-4 h-4 text-[#FF5F03]" /></div><p className={`font-[Oswald] text-[20px] font-semibold ${labaBersih >= 0 ? "text-[#16A34A]" : "text-[#DC2626]"}`}>{formatCurrency(labaBersih)}</p></div>
              <div className="bg-white border border-[#D9D6C8] rounded-lg p-3 border-l-[3px] border-l-[#072C2C]"><div className="flex items-center justify-between mb-1"><p className="text-[10px] font-medium text-[#9CA3AF] uppercase">Margin</p><Calculator className="w-4 h-4 text-[#072C2C]" /></div><p className="font-[Oswald] text-[20px] font-semibold text-[#072C2C]">{totalSales > 0 ? Math.round((labaBersih / totalSales) * 100) : 0}%</p></div>
            </div>
                ...
                <div className="p-4 h-[200px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={(() => {
                      ...
                      filteredTransactions.forEach((t: any) => { const d = new Date(t.created_at); dayMap[days[d.getDay() === 0 ? 6 : d.getDay() - 1]] = (dayMap[days[d.getDay() === 0 ? 6 : d.getDay() - 1]] || 0) + (t.total || 0); });
