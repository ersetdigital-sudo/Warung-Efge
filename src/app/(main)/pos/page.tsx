...
import { Search, Plus, Minus, Trash2, ShoppingCart, CreditCard, QrCode, Banknote, Percent, User, Printer } from 'lucide-react';
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3">
            {filteredProducts.map((product) => (
              <button
                ...
                className="bg-white border border-gray-200 rounded-xl p-3 text-left hover:border-blue-300 hover:shadow-md transition-all group"
              >
              ...
              <div key={item.productId} className="flex items-center gap-3 p-2 bg-gray-50 rounded-lg">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="w-7 h-7 rounded-md bg-white border border-gray-200 flex items-center justify-center hover:bg-gray-100"
                  >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              >
                <Banknote className={`w-5 h-5 ${paymentMethod === 'cash' ? 'text-blue-600' : 'text-gray-400'}`} />
                <span className={`text-xs font-medium ${paymentMethod === 'cash' ? 'text-blue-700' : 'text-gray-600'}`}>Tunai</span>
              </button>
              <button
              ...
              >
                <CreditCard className={`w-5 h-5 ${paymentMethod === 'transfer' ? 'text-blue-600' : 'text-gray-400'}`} />
                <span className={`text-xs font-medium ${paymentMethod === 'transfer' ? 'text-blue-700' : 'text-gray-600'}`}>Transfer</span>
              </button>
              <button
              ...
              >
                <QrCode className={`w-5 h-5 ${paymentMethod === 'qris' ? 'text-blue-600' : 'text-gray-400'}`} />
                <span className={`text-xs font-medium ${paymentMethod === 'qris' ? 'text-blue-700' : 'text-gray-600'}`}>QRIS</span>
              </button>
            </div>
          </div>
          ...
          {selectedCustomer && (
            <label className="flex items-center gap-2 p-3 bg-yellow-50 rounded-lg border border-yellow-200 cursor-pointer">
              ...
              />
              <div>
                <p className="text-sm font-medium text-yellow-800">Catat sebagai Bon/Hutang</p>
                <p className="text-xs text-yellow-600">Pembayaran dilakukan nanti</p>
              </div>
            </label>
          )}
          ...
          {!isDebt && paymentMethod === 'cash' && (
            <div>
              ...
              <input
                ...
                placeholder="0"
                className="w-full px-4 py-3 text-lg border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-right font-bold"
              />
              {Number(amountPaid) >= total && (
                <p className="text-right text-sm mt-1 text-green-600 font-medium">
              ...
              <div className="grid grid-cols-4 gap-2 mt-2">
                {[total, Math.ceil(total / 10000) * 10000, Math.ceil(total / 50000) * 50000, 100000].map((amount) => (
                  <button
                    ...
                    className="px-2 py-1.5 text-xs font-medium bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors"
                  >
          ...
          <Button
            ...
            disabled={!isDebt && paymentMethod === 'cash' && Number(amountPaid) < total}
          >
      ...
      <Modal isOpen={showReceipt} onClose={handleNewTransaction} title="Transaksi Berhasil" size="sm">
        <div className="space-y-4">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500 mt-1">Transaksi telah tercatat</p>
          </div>
            ...
            {!isDebt && paymentMethod === 'cash' && (
              <>
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-green-600">{formatCurrency(change > 0 ? change : 0)}</span>
                </div>
              </>
            )}
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Metode</span>
              <span className="capitalize">{isDebt ? 'Bon/Hutang' : paymentMethod}</span>
            </div>
          </div>
