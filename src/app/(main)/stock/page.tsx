...
export default function StockPage() {
  ...
  const stockColumns = [
    ...
    { key: "stock", label: "Stok", sortable: true, render: (item: any) => { const s = getStockStatus(item.stock, item.min_stock); const bulkDisplay = item.has_bulk_unit && item.bulk_conversion ? ` (${Math.floor(item.stock / item.bulk_conversion)} ${item.bulk_unit})` : ""; const retailDisplay = item.retail_unit && item.retail_conversion ? ` · ${item.stock * item.retail_conversion} ${item.retail_unit}` : ""; return <div><Badge variant={s === "safe" ? "success" : s === "warning" ? "warning" : "danger"}>{formatNumber(item.stock)} {item.unit}{bulkDisplay}</Badge>{retailDisplay && <p className="text-[9px] text-[#9CA3AF] mt-0.5">{retailDisplay}</p>}</div>; } },
