...
import {
  ...
  Zap, Store, Receipt, UserCircle, ClipboardCheck, Play,
} from "lucide-react";
    ...
    <div className="max-w-3xl mx-auto space-y-4 pb-20 md:pb-8">
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-[#9CA3AF] mt-0.5">Panduan lengkap penggunaan Warung Efge</p>
        </div>
      </div>
      ...
      <div className="bg-[#072C2C] rounded-2xl p-4 sm:p-5 flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-white/60 mt-1 leading-relaxed">
            Urutan yang disarankan: <span className="text-white/90 font-medium">Pengaturan</span> → <span className="text-white/90 font-medium">Tambah Produk</span> → <span className="text-white/90 font-medium">Tambah Supplier</span> → <span className="text-white/90 font-medium">Mulai Kasir</span>
          </p>
          <div className="flex flex-wrap gap-2 mt-3">
            {[
              { label: "Pengaturan", href: "/settings", color: "bg-[#FF5F03]" },
              { label: "Tambah Produk", href: "/products/tambah", color: "bg-[#FF5F03]" },
              { label: "Kasir", href: "/pos", color: "bg-[#FF5F03]" },
            ].map(btn => (
              <a key={btn.href} href={btn.href} className={`flex items-center gap-1.5 px-3 py-1.5 ${btn.color} text-white text-xs font-bold rounded-lg hover:opacity-90 transition-opacity`}>
                <Play className="w-3 h-3" />{btn.label}
              </a>
            ))}
          </div>
        </div>
      ...
      <div className="space-y-2">
        {sections.map((sec) => {
          ...
          return (
            <div
              key={sec.id}
              className={`bg-white border rounded-2xl overflow-hidden shadow-sm transition-all duration-200 ${isOpen ? "border-[#E5E3DC] shadow-md" : "border-[#E5E3DC]"}`}
            >
              {/* Header */}
              <button
              ...
              >
                <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl ${sec.bg} flex items-center justify-center flex-shrink-0`}>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className="hidden sm:inline text-[10px] font-semibold text-[#9CA3AF]">{sec.steps.length} langkah</span>
                  {isOpen
                  ...
                  <div className="mt-4 space-y-3">
                    {sec.steps.map((step, idx) => (
                      <div key={idx} className="flex gap-3">
                        {/* Step number */}
                        <div className={`w-6 h-6 rounded-full ${sec.bg} flex items-center justify-center flex-shrink-0 mt-0.5`}>
                          <span className="text-white text-[10px] font-black">{idx + 1}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-[#072C2C] leading-tight">{step.title}</p>
      ...
      <div className="bg-[#EDEADE] border border-[#D9D6C8] rounded-2xl p-4 text-center">
        ...
        <p className="text-xs text-[#9CA3AF]">Warung Efge — POS & Inventory</p>
