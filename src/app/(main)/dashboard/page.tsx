  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader><h3 className="text-base font-semibold text-gray-900">Grafik Penjualan Minggu Ini</h3></CardHeader>
          <CardContent>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={salesChartData}>
                  ...
                  <YAxis axisLine={false} tickLine={false} fontSize={12} tickFormatter={(v) => `${(v / 1000000).toFixed(1)}jt`} />
        ...
        <Card>
          <CardHeader><h3 className="text-base font-semibold text-gray-900">Produk Terlaris</h3></CardHeader>
          <CardContent>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card>
          <CardHeader><h3 className="text-base font-semibold text-gray-900">Hutang Pelanggan</h3></CardHeader>
          <CardContent className="p-0">
            <div className="divide-y divide-gray-100">
              {customers.filter((c) => c.debt > 0).map((customer) => (
                <div key={customer.id} className="flex items-center justify-between px-5 py-3">
                  ...
                  <p className="text-sm font-semibold text-red-600">{formatCurrency(customer.debt)}</p>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader><h3 className="text-base font-semibold text-gray-900">Hutang Supplier</h3></CardHeader>
          <CardContent className="p-0">
            <div className="divide-y divide-gray-100">
              {suppliers.filter((s) => s.debt > 0).map((supplier) => (
                <div key={supplier.id} className="flex items-center justify-between px-5 py-3">
                  ...
                  <p className="text-sm font-semibold text-orange-600">{formatCurrency(supplier.debt)}</p>
                </div>
              ))}
            </div>
          </CardContent>
